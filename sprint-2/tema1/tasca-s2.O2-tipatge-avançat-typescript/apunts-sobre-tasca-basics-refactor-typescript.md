...
```javascript
// JavaScript -- cap error fins que s'executa
const suma = (a, b) => a + b

// TypeScript -- error immediatament si passes strings
const suma = (a: number, b: number) => a + b
suma("hola", "món") // ❌ error en compilació
```
...
string    // → "hola"
boolean   // → true / false
void      // → funció que no retorna res
unknown   // → tipus desconegut
null      // → valor nul explícit 
...
```typescript
// Propietat opcional en objecte
const fn = (params: { first: string; last?: string }) => { }

// Paràmetre opcional
const fn = (first: string, last?: string) => { }
```
...
const user: User = { id: 1, firstName: "Jen", lastName: "Simmons" }
```
...
```typescript
posts: Post[]        // ← sintaxi curta
posts: Array<Post>   // ← sintaxi genèrica
```
...
// Funció normal
const makeUser = (): User => {
  return { id: 1, firstName: "Jen", lastName: "Simmons" }
}

// Si retorna un tipus incorrecte → error
const makeUser = (): User => {
  return "hola"  // ❌ error -- retorna string, no User
}
...
```typescript
// ❌ Error -- async sempre retorna Promise
...
guitarists.add("Jimi Hendrix")  // ✅ és un string
guitarists.add(2)               // ❌ error -- no és un string
```
...
interface BaseEntity {
  id: string;  // ← propietat compartida
}

interface User extends BaseEntity {
  firstName: string;  // ← propietats pròpies
}
...
// → té totes les propietats de User + posts
```
...
// instanceof -- per objectes
if (e instanceof Error) {
  e.message  // ✅ TypeScript sap que és un Error
}
```
